...
class Model20(PreRTModelBase):
    ...
    pknee: ConstParam.using(float, 'Knee pressure (atm)') # noqa: F722 F821
    ptrop: ConstParam.using(float, 'Tropopause cut-off pressure (atm)') # noqa: F722 F821
    ...
    def from_apr_file(
        ...
        #Reading pknee and ptrop from the same line
        s = f.readline().split()
        pknee = float(s[0])
        ptrop = float(s[1])
        ...
        # Legacy APR stored fractional scale height in log space; deep abundance in log for non-temperature profiles
