...
from archnemesis.database.offline.ans_line_data_file import AnsLineDataFile
from archnemesis.database.offline.ans_partition_fn_data_file import AnsPartitionFunctionDataFile
...
class LineData_1:
    ...
    def fetch_linedata(
        ...
        """
        nu_combined = np.concatenate(nu_list)
        nu_sort_idxs = np.argsort(nu_combined)
        
        self.line_data = np.rec.fromarrays(
            (
                np.array((np.concatenate(mol_id_list),np.concatenate(local_iso_id_list)), dtype=int).T[nu_sort_idxs],
                nu_combined[nu_sort_idxs],
                np.concatenate(sw_list)[nu_sort_idxs],
                np.concatenate(a_list)[nu_sort_idxs],
                np.concatenate(gamma_amb_list)[nu_sort_idxs],
                np.concatenate(n_amb_list)[nu_sort_idxs],
                np.concatenate(delta_amb_list)[nu_sort_idxs],
                np.concatenate(gamma_self_list)[nu_sort_idxs],
                np.concatenate(n_self_list)[nu_sort_idxs],
                np.concatenate(elower_list)[nu_sort_idxs],
            ),
            dtype=dtype
        )
        """
