...
DEFAULT_DBASE_NO_DOWNLOAD_PATH = Path(archnemesis_resolve_path(archnemesis_path()+'/archnemesis/Data/reference_databases/hitran_2024.h5.no_download'))
...
def download_default_database():
    ...
    if DEFAULT_DBASE_SENTRY_PATH.exists() and not DEFAULT_DBASE_PATH.exists() and not DEFAULT_DBASE_NO_DOWNLOAD_PATH.exists():
        display(f'ARCHNEMESIS :: Setup :: The default database **is not** present at {DEFAULT_DBASE_PATH!s}.')
        display(f'ARCHNEMESIS :: Setup :: The sentry file **is** present at {DEFAULT_DBASE_SENTRY_PATH!s}.')
        display(f'ARCHNEMESIS :: Setup :: The stop file **is not** present at {DEFAULT_DBASE_NO_DOWNLOAD_PATH!s}.')
        display( 'ARCHNEMESIS :: Setup :: Therefore, downloading default database....')
        safe_download(DEFAULT_DBASE_URL, DEFAULT_DBASE_PATH)
    else:
        if DEFAULT_DBASE_PATH.exists():
            display(f'ARCHNEMESIS :: Setup :: The default database **is** present at {DEFAULT_DBASE_PATH!s}.')
        if not DEFAULT_DBASE_SENTRY_PATH.exists():
            display(f'ARCHNEMESIS :: Setup :: The sentry file **is not** present at {DEFAULT_DBASE_SENTRY_PATH!s}.')
        if DEFAULT_DBASE_NO_DOWNLOAD_PATH.exists():
            display(f'ARCHNEMESIS :: Setup :: The stop file **is** present at {DEFAULT_DBASE_NO_DOWNLOAD_PATH!s}.')
        
        display( 'ARCHNEMESIS :: Setup :: Therefore, **not** downloading default database....')
        display( 'ARCHNEMESIS :: Setup :: ')
        display( 'ARCHNEMESIS :: Setup :: To enable download perform the following steps:')
        display( 'ARCHNEMESIS :: Setup :: \t1) Ensure the default database **is not** present at')
        display(f'ARCHNEMESIS :: Setup :: \t   {DEFAULT_DBASE_PATH!s}')
        display( 'ARCHNEMESIS :: Setup :: \t2) Ensure the sentry file **is** present at')
        display(f'ARCHNEMESIS :: Setup :: \t  {DEFAULT_DBASE_SENTRY_PATH!s}')
        display( 'ARCHNEMESIS :: Setup :: \t3) Ensure the stop file **is not** present at')
        display(f'ARCHNEMESIS :: Setup :: \t  {DEFAULT_DBASE_NO_DOWNLOAD_PATH!s}')
